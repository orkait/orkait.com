...
This GraphQL configuration is enough for starting a GraphQL server using orkait. You can start the server using the [`start`](/docs/orkait-graphql-cli/#start) command.
...
You can set up a GraphQL Server using orkait by following the steps below:

1. Install orkait's GraphQL CLI by running the following command:

```bash
npm install -g @orkaithq/orkait
```
...
Using orkait's GraphQL CLI, you can quickly set up a GraphQL server with minimal configuration. The CLI provides a streamlined development environment for building GraphQL APIs.
...
Lets attach resolvers to the schema we defined in the previous section using orkait's GraphQL Configuration:
...
This configuration is enough for starting a GraphQL server using orkait. You can start the server using the [`start`](/docs/orkait-graphql-cli/#start) command.

For starting the server, you can use the following command:

```bash
orkait start path/to/your-graphql-configuration
```
...
**orkait Playground**: [orkait Playground](https://orkait.run/playground) is a web-based IDE for testing and debugging GraphQL servers,and analyze the behavior of your GraphQL server.
