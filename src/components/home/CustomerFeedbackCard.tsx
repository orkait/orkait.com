...
const CustomerFeedbackCard: React.FC<CustomerFeedbackCardProps> = ({
  ...
  return (
    <div
      className={`customer-feedback-card md:w-[33%] flex flex-col items-center justify-between bg-orkait-dark-600 orkait-light-100 text-white py-SPACE_06 px-SPACE_06 text-center text-content-small gap-y-SPACE_06 pb-10 ${classNames} ${
      ...
      {department && (
        <>
          <span className={`sm:text-content-medium lg:text-content-large !font-bold !text-title-large`}>
            {`Loved by `}
            <span className={isCenterCard ? `text-orkait-yellow` : ""}>{department}</span>
          </span>
        </>
      )}
