...
const LinkButton = ({
    ...
    const generateThemeClasses = () => {
        const themes = {
            [Theme.Light]: {
                classes:
                    "border border-solid border-orkait-border-dark-100 text-orkait-dark-500 bg-transparent hover:text-orkait-dark-500",
                gridClasses: "",
            },
            [Theme.Dark]: {
                classes:
                    "border-2 border-solid border-orkait-border-dark-100 text-orkait-light-100 bg-white hover:text-orkait-light-100",
                gridClasses: "",
            },
            [Theme.Gray]: {
                classes:
                    "border-2 border-solid border-orkait-light-100 text-orkait-light-100 bg-transparent hover:text-orkait-light-100",
                gridClasses: "hidden",
            },
            [Theme.orkait]: {
                classes: "bg-yellow-300 border border-solid text-orkait-dark-500 bg-transparent hover:text-orkait-dark-500",
                gridClasses: "",
            },
        }
        ...
        if (buttonTheme === Theme.Dark || buttonTheme === Theme.Gray) {
            return (
                <>
                    ...
                    <div
                        className={`lg:block rounded-md lg:rounded-lg absolute inset-0 w-full bg-orkait-dark-500 group-hover:lg:scale-x-[0.98] group-hover:lg:scale-y-[0.95] transform transition-all ease-out duration-250`}
                    />
                    {!disabled && (
    ...
    return (
      ...
      ${disabled ? "cursor-not-allowed opacity-20" : ""} ${className ?? ""}`}
            {...props}
        >
