...
const BlogListItem: React.FC<BlogListItemProps> = ({date, title, description, authors, permalink}) => {
  return (
    <Link to={permalink} className="flex flex-col overflow-hidden !text-black !no-underline">
      <div className="flex flex-col flex-1 p-3 md:py-12 md:px-6 gap-2 md:gap-3 border border-solid border-orkait-border-light-400 hover:border-orkait-border-dark-100 rounded-lg md:rounded-md">
        ...
        <div className="flex flex-col flex-1 gap-1 md:gap-2">
          ...
          <span className="flex-1 text-content-tiny md:text-content-small line-clamp-1 md:line-clamp-3 text-orkait-light-600 blog-post-content-desc">
            {description}
          </span>
        </div>
