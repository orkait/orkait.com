...
The `server` configuration provides a comprehensive set of server configurations. It dictates server behavior and helps tune orkait for a range of use-cases.
...
Setting the `port` to `8090` means that orkait will be accessible at `http://localhost:8000`.
...
This example sets the `port` to `8090`, making orkait accessible at `http://localhost:8090`.
...
Allows intelligent configuration of the final response headers that's produced by orkait.

## cacheControl

Activating the `cacheControl` configuration directs orkait to send [Cache-Control] headers in its responses. The `max-age` value in the header matches the lowest of the values in the responses that orkait receives from its upstream. By default, this is `false`, which means orkait does not set any header.
...
When the `experimental` configuration is enabled, orkait can include headers starting with `X-` in its responses, which are sourced from its upstream. By default, this feature is disabled (`[]`), meaning orkait does not forward any such headers unless explicitly configured to do so.
...
Enabling the `setCookies` option instructs orkait to include `set-cookie` headers in its responses, which are obtained from the headers of upstream responses.
...
The `cors` configuration allows you to enable [CORS](https://developer.mozilla.org/en-US/docs/Web/HTTP/CORS) on orkait. This is useful when you want to access orkait in the browser. Here is a simple configuration to get started with cors:
...
In the provided example, setting a variable named `apiKey` with a placeholder value of "YOUR_API_KEY_HERE" implies that whenever orkait fetches data from the `externalData` endpoint, it includes the `apiKey` in the Authorization header of the HTTP request.
...
orkait can automatically infer the schema of the HTTP endpoints for you. This information can check responses received from the upstream services. Enabling this setting allows you to do that. If not specified, the default setting for `responseValidation` is `false`.
...
In this example, the GraphQL endpoint is changed to `/orkait-gql` and the status endpoint to `/health`.

## enableFederation

A boolean flag, if set to `true` the orkait server will additionally act as federation subgraph. By default, it's disabled.
