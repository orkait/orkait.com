...
orkait automatically resolves the protobuf file for any methods referenced in the `@grpc` directive.
...
:::info
The `batchKey` setting helps solve the N+1 problem by grouping similar requests together for optimized batching.
:::

### onResponseBody

This hook allows you to intercept and modify the response body from upstream services before it's processed by orkait. Like [onRequest](./http.md#onrequest), it accepts a string value representing a middleware function defined in a JavaScript file. This function can be used to transform or validate the response data.
