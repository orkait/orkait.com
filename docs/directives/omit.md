...
When applied to a field or node, the `@omit` directive instructs the orkait not to include that field or node in the schema. This means that clients cannot query or mutate data in those fields.
